...
class Slab:
    """
    For slab calculations. Only tested for fcc lattice so far.
    """
    ...
    def __init__(self, 
        ...
        self.adaptedLatticeVectors = [self.meshPrimitives[0], 
                                      ...
                                      self.zPrimitive]
        self.slabCell = self._buildSlabCell()
    ...
    def _getLatticePlanes(self):
        ...
        set_of_latvecs = set(map(tuple, self.lattice.lattice_vectors))
    ...
    def _buildSlabCell(self):
        ...
        Returns
        -------
        slabCell : Lattice object for the whole slab 
        """
        ...
        for lz in range(self.numCells):
            for atom in self.lattice.atoms:
                ...
                slabCell.append((element, fractional_coords))
        
        (s1, s2, s3) = self.adaptedLatticeVectors
        s3 = s3*self.numCells
        
        slab = Lattice(slabCell, (s1, s2, s3))
    ...
    def _cartToFrac(self, vector):
        ...
        delta. Thus if R = c_1 a_1 + c_2 a_2 + c_3 a_3, the fractinoal 
        coefficients ci are found as 
                c_i = R @ sigma_i / (cellVol)

        Parameters
        ----------
        vector : 3-Vector in cartesian coordiantes

        Returns
        -------
        fractional_coords : vector represented in fractional coordinates
        ...
        cellVolume = la.det(slabCellVectors)
